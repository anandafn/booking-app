...
const Search = () => {
  // search variable contains values from user input in the searchBar
  ...
  const searchParams = {
    ...
    checkIn: search.checkIn.toISOString(), // to convert date to string, use toISOString()
  ...
  // get the query data to show to the user
  ...
  const handleHotelFacilitiesChange = (
    ...
    setSelectedHotelFacilities((prevHotelFac) =>
      event.target.checked
        ? [...prevHotelFac, hotelFacility]
        : prevHotelFac.filter((facility) => facility !== hotelFacility)
    );
  };
